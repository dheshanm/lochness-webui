        ...
        <>
                    ...
                    <div className="flex items-center justify-between">
                        <div className="flex items-center">
                            <Button
                                ...
                                className="mr-2"
                            >
                                <Link href={`/config/projects/${projectId}/sites/${siteId}/data-sources/xnat/${instanceName}`} className="flex items-center">
                                    <ChevronLeft />
                                </Link>
                            </Button>
                            <Breadcrumb>
            ...
            <div className="container mx-auto p-6 max-w-5xl flex flex-col h-full">
                <div className="flex items-center gap-2 mb-4">
                    <Image src="/logo/xnat.jpeg" alt="XNAT" width={32} height={32} className="rounded" />
                    <h1 className="text-2xl font-semibold">Update XNAT Data Source</h1>
                </div>
                <div className="flex-grow overflow-auto mt-4">
                    {projectId && siteId && instanceName && <XnatForm project_id={projectId} site_id={siteId} instance_name={instanceName} />}
                    {/* Mask sensitive info in config details if displayed */}
                    {/* Example: <pre>{JSON.stringify({...config, api_token: '••••••••••••••••••••'}, null, 2)}</pre> */}
                </div>
            </div>
        </>
    )
